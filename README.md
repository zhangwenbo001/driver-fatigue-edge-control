...
`firmware/` 包含项目专用的 FreeRTOS 任务、DMS1 解析器、UART 钩子、LED/蜂鸣器驱动以及 Keil 工程文件。本仓库不包含第三方 HAL/CMSIS/FreeRTOS 模板源码和构建产物；重新构建 Keil 工程前，请补充匹配的官方 STM32F1 HAL、CMSIS 和 FreeRTOS 依赖。
...
- 已在 306 张图像的测试集上完成 PT/ONNX 等价性评估；源数据集和人脸图像未包含在本仓库中。
- 已发布模型文件仅供本地评估使用。
- 因尚无 Linux 目标板，尚未完成 Linux/NPU 目标板实测、厂商特定转换、温度和功耗测试；这些工作不被声明为已完成。

## 数据说明

本仓库仅包含可公开的源码、配置与文档；原始数据集、人脸样本图像及内部开发记录不在此公开。